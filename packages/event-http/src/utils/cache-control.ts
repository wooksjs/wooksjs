...
export function renderCacheControl(data: TCacheControl) {
    ...
    for (const [a, v] of Object.entries(data) ) {
        ...
        const func: (v: unknown) => string = cacheControlFunc[a as keyof typeof cacheControlFunc] as (v: unknown) => string
...
const cacheControlFunc = {
    ...
    noCache: (v: TCacheControl['noCache']) => v ? (typeof v === 'string' ? `no-cache="${ v }"` : 'no-cache') : '',
    ...
    private: (v: TCacheControl['private']) => v ? (typeof v === 'string' ? `private="${ v }"` : 'private') : '',
