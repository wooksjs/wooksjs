...
export function renderCacheControl(data: TCacheControl) {
    ...
    for (const [a, v] of Object.entries(data)) {
        ...
        const func: (v: unknown) => string = cacheControlFunc[
            a as keyof typeof cacheControlFunc
        ] as (v: unknown) => string
...
const cacheControlFunc = {
    mustRevalidate: (v: TCacheControl['mustRevalidate']) =>
        v ? 'must-revalidate' : '',
    ...
    proxyRevalidate: (v: TCacheControl['proxyRevalidate']) =>
        v ? 'proxy-revalidate' : '',
    maxAge: (v: Required<TCacheControl>['maxAge']) =>
        'max-age=' + convertTime(v, 's').toString(),
    sMaxage: (v: Required<TCacheControl>['sMaxage']) =>
        's-maxage=' + convertTime(v, 's').toString(),
}
