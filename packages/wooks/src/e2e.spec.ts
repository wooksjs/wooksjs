...
const sendRequest = async (method: string, path: string, body?: string): Promise<IncomingMessage> =>
  new Promise(resolve => {
...
const get = async (path: string): Promise<IncomingMessage> => sendRequest('GET', path)

const post = async (path: string, body: string): Promise<IncomingMessage> =>
