...
describe('Wooks E2E', () => {
    ...
    it('must parse route params', async () => {
        expect(await getBody('params/value1/value%232')).toEqual(
            '["value1","value#2"]'
        )
    })

    it('must parse query params', async () => {
        expect(await getBody('query?p1=value1&p2=value%232')).toEqual(
            '["value1","value#2"]'
        )
    })
    ...
    it('must parse body in post request', async () => {
        ...
        accept = 'application/json'
        expect(
            await getBody('post', JSON.stringify({ a: 'a', b: [1, 2, 3] }))
        ).toEqual('{"a":"a","b":[1,2,3]}')
    })
    ...
    it('must return error 400 when json body is bad', async () => {
        ...
        expect(
            await getBody(
                'post',
                JSON.stringify({ a: 'a', b: [1, 2, 3] }).slice(0, 5)
            )
        ).toEqual(
